...
//generate ground
makeGround(-200,6200,960,760,1,0.5);
makeGrass(75,50,roomWidth - 50);
var trees = [sTree01,sTree02,sTree03,sTree04,sTree05,sTree06,sTree07,sTree08,sTree09,sTree10];
distributeObjects(trees,oBlockLoS,"LoSTrees",500,1000,0,0,roomWidth);
var shrubs = [sS01,sS02, sS03, sS04, sS05, sS06, sS07, sS08, sS09, sS10, sS11, sS12, sS13, sS14, sS15];
distributeObjects(shrubs,oConcealment,"LoSTrees",20,400,.5,0,roomWidth);
...
function makeGrass(count, leftX, rightX)
{
	while(count > 0)
	{
		with(instance_create_layer(irandom_range(leftX,rightX),759,"Poo",oGrowable))
		{
			sprite_index = sGrassYellow;
			image_yscale = random_range(1,3);
			lifeCounter = irandom_range(0,360);
		}
		count--;
	}
